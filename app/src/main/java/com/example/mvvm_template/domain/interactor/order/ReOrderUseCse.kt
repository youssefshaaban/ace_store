...
class ReOrderUseCse @Inject constructor(private val orderRepository: OrderRepository, private val ioDispatcher: CoroutineContext):UseCase<RateDTO,DataState<Boolean>>{
    override fun execute(param: RateDTO): Flow<DataState<Boolean>> {
        return flow {
            emit(orderRepository.reOrder(param))
        }.flowOn(ioDispatcher)
    }
}