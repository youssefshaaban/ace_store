...
class GetOrderByIdUseCse @Inject constructor(private val orderRepository: OrderRepository, private val ioDispatcher: CoroutineContext):UseCase<Int,DataState<Order>>{
    override fun execute(param: Int): Flow<DataState<Order>> {
        return flow {
            emit(orderRepository.getOrderById(param))
        }.flowOn(ioDispatcher)
    }
}