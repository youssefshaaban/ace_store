...
class ContinueCompleteActivity : BaseActivity<ActivitySignUpBinding>() {
    ...
    private fun registerOnclickListner() {
        getViewDataBinding().register.setOnClickListener {
            if (validateInput()) {
                SavePrefs(this,User::class.java).save(user!!)
                viewModel.completeRegister(UpdateProfileUseCase.UpdateRequestProfile(
    ...
    companion object {
        fun getIntent(context: Context): Intent = Intent(context, ContinueCompleteActivity::class.java)
    }
}