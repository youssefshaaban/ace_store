...
public class MaskedEditText
  ...
  public MaskedEditText(Context context, AttributeSet attr, String mask, char placeholder) {
    ...
    for (int i = 0; i < N; ++i) {
      ...
      if (at == R.styleable.MaskedEditText_mask) {
        ...
        placeholder =
            (a.getString(at).length() > 0 && placeholder == ' ' ? a.getString(at).charAt(0)
                : placeholder);
      }
    }
    ...
    if (mask.length() > 0) {
      setText(getText()); // sets the text to call the mask
    }
  }
    ...
    for (char maskChar : mask.toCharArray()) {
      if (stack.length() == 0 && !isMaskChar(maskChar)) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
          builder
              .append(String.valueOf(maskChar), new LiteralSpan(),
  ...
  private boolean matchMask(char mask, char value) {
    ...
    ret = ret || (mask == ALPHANUMERIC_MASK && (Character.isDigit(value) || Character
        .isLetter(value)));
  ...
  public void resetSelection() {
    if (selectionSpan != null) {
      SpannableStringBuilder value = new SpannableStringBuilder(getText());
      Selection
          .setSelection(value, value.getSpanStart(selectionSpan), value.getSpanEnd(selectionSpan));
    }
  }
